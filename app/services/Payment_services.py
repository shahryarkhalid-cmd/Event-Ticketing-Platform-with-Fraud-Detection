...
def create_checkout_session(order_id: int, user: User, session: Session):
    ...
    checkout_session = stripe.checkout.Session.create(
        ...
        mode="payment",
        success_url="http://localhost:3000/payment-result.html?status=success&order_id=" + str(order.id),
        cancel_url="http://localhost:3000/payment-result.html?status=cancel&order_id=" + str(order.id),
